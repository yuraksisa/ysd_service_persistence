    ...
    def self.included(base)
      ...
      base.class_eval do
        class << self
          ...
          #
          # Hook for the model class : When a method is added
          #	It's a ruby standard hook
          #
          ...
          #
          # Hook for the model class : When a class model is added
          #
          # It's a ruby standard hook
          #
    ...
    module ClassMethods
      ...
      def process_method_added(method_name, scope)
        hooks_with_scope(scope).each do |target_method, hooks|
          # If a before hook has been configured using Persistence::Model::ModelHook	
          if hooks[:before].any? { |hook| hook[:name] == method_name }
            define_hook_stack_execution_methods(target_method, scope)
          end
          # If a after hook has been configured using Persitence::Model::ModelHook
